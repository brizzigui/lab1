...
int main(){
    ...
    if (temIdaEVolta == 's'){
        valorFinal = valorTotal * 2 + 1.5 * numeroDePassagens;
    }
    ...
    if (temIdaEVolta == 's')
        printf("Valor Final: R$%.2f -> ida e volta -> (R$%.2f x 2 + taxas)", valorFinal, valorTotal);
    ...
    if (temIdaEVolta == 's'){
        percentualDiferenca = valorFinal / (valorBaseUnitario*numeroDePassagens*2);
    }
